...
contract('FalconsToken', function (accounts) {
  ...
  it('initializes the contract with the correct values', function () {
    return FalconsToken.deployed().then(function (instance) {
      ...
      assert.equal(name, 'Falcons Token', 'has the correct name');
      return tokenInstance.symbol();
    }).then(function (symbol) {
      assert.equal(symbol, 'FT', 'has the correct symbol');
      return tokenInstance.standard();
    }).then(function (standard) {
      assert.equal(standard, 'Falcons Token v1.0', 'has the correct standard');
    });
  })
